    ...
    <div className="max-w-6xl mx-auto bg-white p-8 rounded-lg shadow-lg space-y-10">
      ...
      {dadosDoAno && (
        <section className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Card><CardContent className="text-center"><p className="text-sm">Yield Projetado</p><p className="text-xl font-bold">{(dadosDoAno.yield_projetado * 100).toFixed(2)}%</p></CardContent></Card>
          <Card><CardContent className="text-center"><p className="text-sm">Crescimento</p><p className="text-xl font-bold">{(dadosDoAno.crescimento_estimado_lucro * 100).toFixed(2)}%</p></CardContent></Card>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          <h3 className="text-lg font-semibold mb-2">📈 Lucro Líquido Anual</h3>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={[...dados_financeiros].reverse()}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="ano" />
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="lucro_liquido_anual" stroke="#2563eb" />
            </LineChart>
          </ResponsiveContainer>
        </div>
        <div>
          <h3 className="text-lg font-semibold mb-2">💸 Dividendos por Ação</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={[...dadosComAnoAnterior].reverse()}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis dataKey="ano" />
              <YAxis />
              <Tooltip />
              <Bar dataKey="dividendo_ano_anterior" fill="#4ade80" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
