    ...
    <div className="max-w-5xl mx-auto bg-white p-8 rounded-lg shadow-lg space-y-10">
      ...
      {dadosDoAno && (
        <section className="bg-blue-50 p-6 rounded shadow-sm max-w-md mx-auto">
          ...
          <p><strong>Cotação YF:</strong> R$ {dadosDoAno.cotacao_yf.toFixed(2)}</p>
          <p><strong>Preço Teto:</strong> R$ {dadosDoAno.preco_teto.toFixed(2)}</p>
          <p><strong>Crescimento Estimado:</strong> {(dadosDoAno.crescimento_estimado_lucro * 100).toFixed(2)}%</p>
          <p><strong>Yield Projetado:</strong> {(dadosDoAno.yield_projetado * 100).toFixed(2)}%</p>

         {typeof dadosDoAno.margem === "number" && (
            <p>
              <strong>Margem:</strong>{" "}
              <span
                className={`inline-block px-2 py-1 text-xs rounded font-semibold ${
                  dadosDoAno.margem >= 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                }`}
              >
                {(dadosDoAno.margem * 100).toFixed(2)}%
              </span>
            </p>
          )}

          <p className="mt-4 font-semibold text-blue-600">📌 Recomendação: {dadosDoAno.recomendacao_temp || "N/A"}</p>
        </section>
      )}
